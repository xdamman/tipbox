var TipView = function (options) {
  ...
  var $pgpFingerprint = this.$el.querySelector('#pgp_fingerprint')
  ...
  this.invalidFingerprint = function () {
    $pgpFingerprint.classList.remove('hidden')
    $pgpFingerprint.querySelector('span').textContent = '(INVALID FINGERPRINT)'
  ...
  this.showFingerprint = function (fingerprint) {
    $pgpFingerprint.classList.remove('hidden')
    $pgpFingerprint.querySelector('span').textContent = '0x' + fingerprint.substr(32) + ''
    $pgpFingerprint.querySelector('.padlock_svg').style.fill = '#' + fingerprint.substr(34)
    $pgpFingerprint.classList.toggle('encrypted')
    $pgpFingerprint.setAttribute('title', 'PGP fingerprint:\n' + fingerprint)
  }
