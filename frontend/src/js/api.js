...
var Api = function (options) {
  ...
  var request = function (path, data, fn) {
    ...
    kbpgp.box(params, function (err, encryptedData) {
      if (err) {
        console.error(err)
        throw (err)
      }
      self.emit('request_encrypted', encryptedData)
      var body = JSON.stringify({ encrypted: encryptedData })
      console.info('PGP request length: ', body.length)
      console.info('Sending data: ', body)
      xhr.send(body)
    })
  ...
  kbpgp.KeyManager.import_from_armored_pgp({ armored: options.pgp }, function (err, publicKey) {
    if (err) return console.error('TipboxApi> Cannot init pgp key', options.pgp, publicKey, err)
    self.pgp_key = publicKey
  })
