...
module.exports = function (app) {
  return {
    ...
    create: function (req, res, next) {
      ...
      if (!req.body.recipient || !utils.isEmail(req.body.recipient)) { return res.status(400).send({ code: 401, error: 'Recipient must be a valid email address' }) }

      if (!req.body.subject) { return res.status(400).send({ code: 401, error: 'Subject cannot be empty' }) }
